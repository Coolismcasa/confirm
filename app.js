...
(function initScrollReveal() {
  ...
  function observeAll() {
    const targets = document.querySelectorAll(
      '.reveal, .reveal-left, .reveal-right, .reveal-scale, .stagger, .text-reveal, .text-reveal-words'
    );
    targets.forEach(t => {
      if (!t.classList.contains('in-view')) observer.observe(t);
    });
  }

  window.addEventListener('load', () => setTimeout(observeAll, 300));
  document.addEventListener('click', () => setTimeout(observeAll, 200));
  document.addEventListener('scroll', () => setTimeout(observeAll, 200), { passive: true, once: true });
})();
